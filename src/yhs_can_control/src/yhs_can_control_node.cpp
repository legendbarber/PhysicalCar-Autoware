...
#include <geometry_msgs/msg/transform_stamped.hpp> // <--- TF 메시지 헤더 추가
// <tf2_ros/transform_broadcaster.h> 는 .hpp에 추가했을 것입니다.
...
: node_(node),if_name_("can2"), can_socket_(-1),wheel_base_(0.6), publish_tf_(false)
{
    ...
    ctrl_cmd_subscriber_ = node_->create_subscription<std_msgs::msg::Float64MultiArray>("/tod_cmd", 1, std::bind(&CanControl::ctrl_cmd_callback, this, std::placeholders::_1));
...
void CanControl::ctrl_cmd_callback(const std_msgs::msg::Float64MultiArray::SharedPtr ctrl_cmd_msg)
{
    std_msgs::msg::Float64MultiArray msg = *ctrl_cmd_msg;
    const unsigned short vel = msg.data[0] * 1000;
    const short angular = msg.data[2] * 100;
    const unsigned char gear = msg.data[3];
    const unsigned char brake = msg.data[1];
...
void CanControl::publish_odom(const double velocity, const double steering) 
{
    ...
    t.transform.translation.z = 0.0; // Z축은 0으로 가정

    tf2::Quaternion quat_tf; // Odom 메시지 변수와 충돌 피하기
    quat_tf.setRPY(0.0, 0.0, theta_);
    t.transform.rotation.x = quat_tf.x();
    t.transform.rotation.y = quat_tf.y();
    ...
    // 이전에 quat 변수가 TF용으로 사용되었으므로,
    // odom 메시지용으로 새 변수(또는 tf2::toMsg) 사용
    tf2::Quaternion quat_msg;
    quat_msg.setRPY(0.0, 0.0, theta_);
    pose_cov.pose.orientation.x = quat_msg.x();
    pose_cov.pose.orientation.y = quat_msg.y();
    pose_cov.pose.orientation.z = quat_msg.z();
    pose_cov.pose.orientation.w = quat_msg.w();
